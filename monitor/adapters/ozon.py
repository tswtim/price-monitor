"""ozon.ru adapter — Playwright with persistent profile."""
import re, os
...
class OzonAdapter(BaseAdapter):
    def fetch(self) -> list[RawProduct]:
        ...
        with sync_playwright() as pw:
            ...
            if any(w in text.lower() for w in ["проверк","капч","captcha","запрещен","соединени"]):
                print("   [!] Блокировка — реши капчу в окне браузера и запусти скрипт снова")
            ...
            for i, cat in enumerate(categories):
                ...
                items = page.evaluate("""() => {
                    const cards = [...document.querySelectorAll('[class*="product"], [class*="tile"], [class*="item"]')];
                    return cards.map(c => {
                        const t = c.querySelector('[class*="title"], [class*="name"], span')?.innerText?.trim() || '';
